...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'perai to fazendo garaio😡',
				success: '✔️pronto✔️',
				error: {
					stick: '❌ocorreu um erro ao converter a imagem em figurinha❌',
					Iv: '❌link inválido❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos❌',
					ownerG: '❌esse comando só pode ser usado pelo dono do bot❌',
					ownerB: '❌esse comando só pode ser usado pelo proprietário do bot❌',
					admin: '❌esse comando só pode ser usado por um adm do grupo❌',
					Badmin: '❌esse comando só pode ser usado quando eu virar adm porra❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5514996697232@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'cade o texto garai?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('a maior parte do texto é merda tiw')
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* 𝐓͢𝐈𝐎 𝐈𝐍𝐔𝐌𝐀𝐊𝐈 ϟ\n wa.me/+5514996697232\n*INSTA:* to sem aff kk'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ih ala, o cara bate pra 2d cara viado😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'pinto':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️que?? pinto???'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('o que você está procurando? cu?')
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('cadê o url?')
        ...
        case 'promote':
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('bem vindo ao grupo')
						welkom.push(from)
